...
def find_cointegrated_pairs(data): # Takes in an array of dataframes, outputs cointegrated pairs
	# Count of total tests to be conducted for p-value correction
    total_stocks = 0
    for df in data:
        total_stocks = total_stocks + len(df.columns)
...
def before_trading_start(context, data):
    ...
    for industry in industry_codes:
        ...
        industry_df = []
        industry_df.append( df.loc[df.industry_code == industry,])
        # get array of pairs from this industry_df, using cointegration function
        pairs_array = find_cointegrated_pairs(industry_df)
        print pairs_array[0]
    ...
    # prices_df = get_pricing(symbol_list, fields=['price'], start_date='',end_date='')['price']
